...
static void StepRunLoop_Core()
{
	...
	{
		input_process();
		MpAp_OverrideInput();   // Platinum autopilot: scripted test input
	...
	inFrameBoundary = false;
	{
		Lock lock;
		NDS_exec<false>();
		MpBridge_Pump();        // Platinum mailbox bridge: RAM coherent here
		SPU_Emulate_user();
...
static void StepRunLoop_Paused()
{
	...
	// Platinum: keep the lobby transport alive with no ROM running (menu
	// host/join before loading a game). ~20Hz here; the pump's ROM-side
	// work self-gates on the discovery block, which needs a running game.
	MpBridge_Pump();
...
int _main()
{
	...
	MpBridge_InitFromEnv();   // Platinum mailbox bridge + autopilot (env-gated)
...
//	struct configured_features my_config;
  ...
  video.height = GPU_FRAMEBUFFER_NATIVE_HEIGHT * 2;
	...
	MpBridge_InstallMenu(MainWindow->getHWnd());   // Platinum: Multiplayer menu (after the bar is attached)
	...
	MpBridge_Shutdown();   // Platinum mailbox bridge: close sockets
...
//	testelf();
	...
	// Platinum MP: never default the debug console on — players get the
	// bridge/autopilot printf spam on first launch otherwise. The Tools menu
	// toggle still works for debugging, and harness stdout redirection is
	// unaffected (redirected stdout bypasses the console entirely).
...
DOKEYDOWN:
	...
	case WM_INITMENUPOPUP:
		MpBridge_OnInitPopup((HMENU)wParam);   // Platinum: refresh LAN game list
		break;

	case WM_COMMAND:
		if ((HIWORD(wParam) == 0) && MpBridge_HandleCommand(LOWORD(wParam)))
			return 0;   // Platinum Multiplayer menu
